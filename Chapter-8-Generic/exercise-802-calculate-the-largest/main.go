...
func main() {
	...
	largestRanchStockInt := FindLargestRanchStock(animalStock)
	fmt.Printf("The largest stocked item on the rach is %s\n", largestRanchStockInt)
	largestRanchStockFloat := FindLargestRanchStock(miscStock)
	fmt.Printf("The largest stocked item on the rach is %s\n", largestRanchStockFloat)
}
